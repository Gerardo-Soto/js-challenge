function findParts(schematic) {
    ...
    lines.forEach(line => {
        ...
        for (let index = 0; index < line.length; index++){
            ...
            // part 1
            if (Number.isInteger(parseInt(line[index]))){
                // calculate number
                number *=10
                number +=parseInt(line[index])
                let upLeft, upCenter, upRight, centerLeft, centerRight, downLeft, downCenter, downRight;
                try{
                    upLeft = (lines[line_n-1]).split('')[index-1]
                } catch {}
                try{
                    upCenter = (lines[line_n-1]).split('')[index]
                } catch {}
                try{
                    upRight = (lines[line_n-1]).split('')[index+1]
                } catch {}
                try{
                    centerLeft = (lines[line_n]).split('')[index-1]
                } catch {}
                try{
                    centerRight = (lines[line_n]).split('')[index+1]
                } catch {}
                try{
                    downLeft = (lines[line_n+1]).split('')[index-1]
                } catch {}
                try{
                    downCenter = (lines[line_n+1]).split('')[index]
                } catch {}
                try{
                    downRight = (lines[line_n+1]).split('')[index+1]
                } catch {}
                //console.log(upLeft, upCenter, upRight, centerLeft, centerRight, downLeft, downCenter, downRight);
                
                if (
                    upLeft !== '.' && !Number.isInteger(parseInt( upLeft)) && upLeft !== undefined||
                    upCenter !== '.' && !Number.isInteger(parseInt(upCenter)) && upCenter !== undefined ||
                    upRight  !== '.' && !Number.isInteger(parseInt(upRight)) && upRight !== undefined ||
                    centerLeft  !== '.' && !Number.isInteger(parseInt(centerLeft)) && centerLeft !== undefined ||
                    centerRight !== '.' && !Number.isInteger(parseInt(centerRight)) && centerRight !== undefined ||
                    downLeft !== '.' && !Number.isInteger(parseInt(downLeft)) && downLeft !== undefined ||
                    downCenter !== '.' && !Number.isInteger(parseInt(downCenter)) && downCenter !== undefined ||
                    downRight !== '.' && !Number.isInteger(parseInt(downRight)) && downRight !== undefined
                ) {
                    //console.log(`there is a symbol`);
                    hasASymbol = true;
                    
                } 

                if (upLeft == '*' ){
                    coorsKey = (line_n - 1) +','+ (index - 1);
                    hasAGear = true;
                } else if(upCenter  == '*'){
                    coorsKey = (line_n - 1) +','+ (index);
                    hasAGear = true;
                } else if(upRight  == '*'){
                    coorsKey = (line_n - 1) +','+ (index + 1);
                    hasAGear = true;
                } else if(centerLeft == '*'){
                    coorsKey = (line_n) +','+ (index - 1);
                    hasAGear = true;
                } else if(centerRight == '*'){
                    coorsKey = (line_n) +','+ (index + 1);
                    hasAGear = true;
                ...
                //console.log(`up: ${(lines[line_n-1]).split('')[index]}  dw: ${(lines[line_n+1]).split('')[index]}  le: ${(lines[line_n]).split('')[index-1]}  ri: ${(lines[line_n]).split('')[index+1]}  1q: ${(lines[line_n-1]).split('')[index-1]}  2q: ${(lines[line_n-1]).split('')[index+1]}  3q: ${(lines[line_n+1]).split('')[index-1]}  4q: ${(lines[line_n+1]).split('')[index+1]}`);                
            } else {
                // end of number
                
                if (number > 0 && hasASymbol){
                    amount += number
                    numbersInRow.push(number)

                    if (hasAGear){
                        if (gearsParts[coorsKey] === undefined ) {
                            gearsParts[coorsKey] = []
                            gearsParts[coorsKey].push(number);
                        } else {

                            gearsParts[coorsKey].push(number);
                        }
                    }
                    //console.log(number, amount, hasASymbol);
                    number = 0;
                    hasASymbol = false;       
                    hasAGear = false;                      
        ...
        line_n += 1;
        // new line, if the line ends with a number, add and clean with 0
        if (hasASymbol) {
            amount += number;
        ...
        number = 0;
        hasASymbol = false;
        //console.log(numbersInRow, amount);
        
    });

    console.table(gearsParts)

    console.log(`Amount: ${amount}`);
    console.log(`Gears: ${gears}, length dict: ${Object.keys(gearsParts).length}`);


    let rotations = 0
    for (const key in gearsParts) {
        if (gearsParts[key].length == 2) {
            //console.log(gearsParts[key]);
            rotations += gearsParts[key][0] * gearsParts[key][1];
            
        }
    }
    console.log(`2nd problem: rotations: ${rotations}`);
...
//schematic   4361
//schematic2   553 825      mio> 554 826

const solution = findParts(schematic2);
