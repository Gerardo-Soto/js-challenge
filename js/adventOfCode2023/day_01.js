function CalibrationValues(coors) {
    const lines = coors.split('\n');
    ...
    lines.forEach(line => {
        ...
        for (let character = 0; character < line.length; character++) {
            const number =  parseInt(line[character]);
        ...
        if (secondCoord) calibration += (firstCoord * 10) + secondCoord;
        else calibration += (firstCoord * 10) + firstCoord;
        // sum both values
    });
    return calibration;
}
...
console.time();
console.log(CalibrationValues(map));
console.timeEnd()
