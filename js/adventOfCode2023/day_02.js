 ...
 * 
 */
function wouldHaveBeenPossibleGame(params, rules) {
    const rulesObject = {}
    let cubes = {
        /*red: 0,
        green: 0,
    ...
    let match;
    const regex = /(\d+)\s+(\w+)/g
    let sumOfIdGamesPossibles = 0;

    while ((match = regex.exec(rules)) !== null) {
        const quantity = parseInt(match[1], 10);
        const color = match[2];
        rulesObject[color] = quantity;
    }
    console.log(`Rules objects:a`);
    console.table(rulesObject);
    
    
    let games = params.split('\n')
    //console.log(`game 1: ${games[1]}`);
    let gameId = games[1].split(':');
    let taken = gameId[1].split(';')
    //console.log(taken);
    //let gamesNotPossible = 0;
    //let totalGames = -1;
    let exit = false;
    let newGame = true
    let id;
    games.forEach(game => {
        ...
        while ((match = regex.exec(game)) !== null) {
            console.log(`take: ${match}`);
            
            const quantity = parseInt(match[1], 10);
            const color = match[2];
            
            // check rules
            id = parseInt(game.split(' ')[1].replace(',',''))
            if (rulesObject[color] < quantity && exit == false) {
                console.log(`rulesObject[${color}] < quantity:  ${rulesObject[color]} < ${quantity} - id: ${id}`);
        ...
        if (!exit && newGame) {
            sumOfIdGamesPossibles += id;
            //console.log(`///id: ${id}, sum: ${sumOfIdGamesPossibles} ${typeof(sumOfIdGamesPossibles)}`);
            newGame = false;
        }
        exit = false;
        //totalGames += 1;
    ...
    console.log(sumOfIdGamesPossibles);
    
    return (sumOfIdGamesPossibles);
}

let input = `
Game 1: 3 blue, 1 red; 1 red, 2 green, 2 blue; 2 green
Game 2: 1 blue, 1 green; 3 green, 4 blue, 1 red; 1 green, 1 blue`
...
let rule = '12 red, 13 green, 14 blue'

const solution = wouldHaveBeenPossibleGame(input, rule);
console.log(`Games possibles: ${solution}`);
